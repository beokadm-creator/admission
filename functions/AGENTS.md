...
- 공용 사용자 흐름은 인증, request lock, rate limit를 함께 고려한다.
- AlimTalk 발송 실패가 전체 흐름을 막지 않도록 처리한다.
<!-- STATIC:END -->

<!-- DYNAMIC:START -->
## Dynamic Notes

- 현재 queue 관련 export는 `functions/src/firestoreQueue.ts`에서 재export 된다.
- Functions 런타임은 Node.js 22다.
<!-- DYNAMIC:END -->

## Changelog

- 2026-04-02: 실제 backend 구조에 맞춘 slim guide로 재작성.
