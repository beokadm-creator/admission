...
const TEMP_NOTICE_PHONE = '02-6959-3872';
...
export default function SmartQueueGate() {
  ...
  const useTemporaryNoticePage = schoolId === 'seoul';

  if (useTemporaryNoticePage) {
    return (
      <>
        ...
        <div className={`min-h-screen bg-snu-gray px-4 py-6 text-snu-text sm:px-6 sm:py-8${emergencyBannerActive ? ' pt-12' : ''}`}>
          <div className="mx-auto max-w-5xl">
            <section className="relative overflow-hidden rounded-[2rem] border border-snu-blue/10 bg-white shadow-[0_24px_80px_rgba(0,59,113,0.12)]">
              ...
              <div className="relative px-6 py-8 sm:px-10 sm:py-12 lg:px-14">
                <div className="flex items-start gap-4 border-b border-gray-100 pb-8 sm:gap-5">
                  <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-[1.5rem] border border-snu-blue/10 bg-white p-3 shadow-sm sm:h-24 sm:w-24">
                    <img
                      src={schoolConfig?.logoUrl || '/logo.png'}
                      alt={`${schoolConfig?.name || '서울대학교'} 로고`}
                ...
                <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,0.9fr)]">
                  <section className="order-2 rounded-[1.75rem] border border-snu-blue/10 bg-[linear-gradient(180deg,rgba(0,59,113,0.04),rgba(255,255,255,0.96))] p-6 sm:p-8 lg:order-1">
          ...
          {showProgramImage && (
            <div className="fixed inset-0 z-[100] bg-black/70 px-4 py-6 backdrop-blur-[2px]" onClick={() => setShowProgramImage(false)}>
