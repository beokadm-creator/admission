...
export default function SchoolSettings() {
  ...
  const [forceRound, setForceRoundState] = useState<'round1' | 'round2' | null>(null);
  ...
  useEffect(() => {
    ...
    const loadSchool = async () => {
      try {
        ...
        // 긴급 관리 상태 로드
        setEmergencyNoticeText(data.emergencyNotice?.message || '');
        setEmergencyNoticeEnabled(!!data.emergencyNotice?.enabled);
        setForceRoundState((data.forceActiveRound as 'round1' | 'round2' | null) || null);
      } catch (error) {
  ...
  const handleAdminAction = async (action: string) => {
    ...
    try {
      const fn = httpsCallable(functions, 'runAdminQueueAction');
      await fn({ schoolId, action });
      setActionMessage(`✅ ${action === 'recalculateState' ? '수치 재계산' : '만료 세션 정리'} 완료`);
    } catch (error: any) {
      setActionMessage(`❌ 오류: ${error?.message || '알 수 없는 오류'}`);
    } finally {
      setActionLoading(null);
    }
  };

  const handleSaveEmergencyNotice = async (enabled: boolean) => {
    if (!schoolId) return;
    try {
      await setDoc(doc(db, 'schools', schoolId), {
        emergencyNotice: { enabled, message: emergencyNoticeText }
      }, { merge: true });
      setEmergencyNoticeEnabled(enabled);
      alert(enabled ? '긴급 공지가 활성화되었습니다. 사용자 화면 상단에 즉시 표시됩니다.' : '긴급 공지가 비활성화되었습니다.');
    } catch (error: any) {
      alert('저장 실패: ' + (error?.message || '오류'));
    }
  };

  const handleSetForceRound = async (round: 'round1' | 'round2' | null) => {
    if (!schoolId) return;
    try {
      await setDoc(doc(db, 'schools', schoolId), { forceActiveRound: round }, { merge: true });
      setForceRoundState(round);
      alert(round ? `${round === 'round1' ? '1차' : '2차'} 차수가 강제 활성화되었습니다.` : '차수 강제 설정이 해제되었습니다.');
    } catch (error: any) {
      alert('저장 실패: ' + (error?.message || '오류'));
    }
  };
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-orange-200 bg-orange-50 p-6 shadow-sm">
              <h2 className="mb-4 flex items-center text-lg font-semibold text-orange-900">
                <AlertTriangle className="mr-2 h-5 w-5 text-orange-600" />
                긴급 대응 도구
              </h2>

              {actionMessage && (
                <p className="mb-4 rounded-lg bg-white px-4 py-2 text-sm font-medium text-gray-800 shadow-sm">{actionMessage}</p>
              )}
              ...
              <div className="mb-6 flex flex-wrap gap-3">
                <button
                  type="button"
                  disabled={actionLoading === 'recalculateState'}
                  onClick={() => void handleAdminAction('recalculateState')}
                ...
                >
                  {actionLoading === 'recalculateState' ? '처리 중...' : '🔄 수치 재계산'}
                </button>
                <button
                  type="button"
                  disabled={actionLoading === 'expireStaleReservations'}
                  onClick={() => void handleAdminAction('expireStaleReservations')}
                  className="rounded-xl bg-amber-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-amber-700 disabled:opacity-50"
                >
                  {actionLoading === 'expireStaleReservations' ? '처리 중...' : '🧹 만료 세션 정리'}
                </button>
              </div>

              {/* 차수 강제 전환 */}
              <div className="mb-6 rounded-xl border border-orange-200 bg-white p-4">
                <p className="mb-3 text-sm font-semibold text-gray-900">차수 강제 전환</p>
                <p className="mb-3 text-xs text-gray-500">현재: <strong>{forceRound ? (forceRound === 'round1' ? '1차 강제' : '2차 강제') : '자동 (시간 기반)'}</strong></p>
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => void handleSetForceRound(null)}
                    className={`rounded-lg border px-4 py-2 text-sm font-bold transition ${
                      !forceRound ? 'bg-gray-800 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'
                    }`}>자동 (해제)</button>
                  <button type="button" onClick={() => void handleSetForceRound('round1')}
                    className={`rounded-lg border px-4 py-2 text-sm font-bold transition ${
                      forceRound === 'round1' ? 'bg-blue-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'
                    }`}>1차 강제</button>
                  <button type="button" onClick={() => void handleSetForceRound('round2')}
                    className={`rounded-lg border px-4 py-2 text-sm font-bold transition ${
                      forceRound === 'round2' ? 'bg-rose-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'
                    }`}>2차 강제</button>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-orange-200 bg-white p-4">
                <p className="mb-2 text-sm font-semibold text-gray-900">긴급 공지 배너</p>
                <p className="mb-3 text-xs text-gray-500">활성화하면 사용자 게이트 화면 최상단에 즉시 빨간색 배너로 표시됩니다.</p>
                <textarea
                  rows={2}
                  value={emergencyNoticeText}
                  onChange={(e) => setEmergencyNoticeText(e.target.value)}
                  placeholder="긴급 공지 내용을 입력하세요."
                  className="mb-3 block w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100"
                />
                <div className="flex gap-2">
                  <button type="button" onClick={() => void handleSaveEmergencyNotice(true)}
                    className={`rounded-xl px-4 py-2 text-sm font-bold transition ${
                      emergencyNoticeEnabled ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-gray-200 text-gray-700 hover:bg-red-600 hover:text-white'
                    }`}>
                    {emergencyNoticeEnabled ? '🔴 공지 활성화 중' : '공지 ON'}
                  </button>
                  {emergencyNoticeEnabled && (
                    <button type="button" onClick={() => void handleSaveEmergencyNotice(false)}
