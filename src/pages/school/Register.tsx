...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    const validateSession = async () => {
      try {
        ...
        setSoftNotice('신청 세션을 확인하지 못했습니다. 다시 입장 페이지에서 시작해 주세요.');
  ...
  const onSubmit = async (data: RegisterFormInputs) => {
    ...
    if (!termsAgreed.privacy || !termsAgreed.thirdParty || !termsAgreed.sms) {
      setSoftNoticeTone('error');
      setSoftNotice('필수 약관에 모두 동의해야 신청을 완료할 수 있습니다.');
      return;
    }
    ...
    try {
      ...
      if (getFirebaseError(error)?.code === 'functions/deadline-exceeded') {
        localStorage.removeItem(`registrationSessionId_${schoolId}`);
        localStorage.removeItem(`registrationExpiresAt_${schoolId}`);
        setSoftNoticeTone('error');
        setSoftNotice('작성 시간이 만료되었습니다. 다시 대기열에서 입장해 주세요.');
        navigate(`/${schoolId}/gate`);
      } else if (getFirebaseError(error)?.code === 'functions/failed-precondition') {
        setSoftNoticeTone('error');
        setSoftNotice('현재 신청할 수 없는 상태입니다. 다시 대기열에서 입장해 주세요.');
        navigate(`/${schoolId}/gate`);
